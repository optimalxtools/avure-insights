...
// helper: pick a direct child by component type
  ...
  return (
    React.Children.toArray(children).find(
      (child): child is React.ReactElement<P> =>
        React.isValidElement(child) && child.type === Comp
    ) ?? null
  )
}
...
>(
  (
    ...
    const state: "expanded" | "collapsed" = isMobile
      ? "expanded"
      : open
        ? "expanded"
        : "collapsed";
...
>(
  (
    ...
    if (isMobile) {
      const header = pickChild(children, SidebarHeader)
      const content = pickChild(children, SidebarContent)
      const footer = pickChild(children, SidebarFooter)
        ...
        <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
          <SheetContent
            ...
            className={cn(
              ...
              "[&_[data-sidebar=menu-button]]:h-11",
              "[&_[data-sidebar=menu-button]]:text-base",
              "[&_[data-sidebar=menu-button]>svg]:size-5",
              "[&_[data-sidebar=menu-sub-button]]:h-10",
              "[&_[data-sidebar=menu-sub-button]]:text-base",
              "[&_[data-sidebar=input]]:h-10",
              "[&_[data-sidebar=input]]:text-base"
            )}
            style={{
              "--sidebar-width": "18rem",
              height: "100%",
            ...
            side={side}
          >
            {/* Fallback: if any block wasn't found, just render children so it's never blank */}
...
const SidebarContent = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(
  ({ className, ...props }, ref) => (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-2 pb-[calc(env(safe-area-inset-bottom)+4.5rem)]",
        "group-data-[collapsible=icon]:overflow-hidden", // keep for desktop-collapsed rail
        className
      )}
      {...props}
    />
